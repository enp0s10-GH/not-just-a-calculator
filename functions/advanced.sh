#!/bin/bash
#
# This functions checks the input, calculates and the converted Output

########################################
# function to convert decimal, binary, hexadecimal and text.
# Output: 
#   Result of the Input 
#   Error message if the Input isnt valid 
########################################
function advanced() {
  ...
  if [[ "${convertfrom}" == '1' ]]; then
    ...
    if [[ $binary_value =~ ^[0-1]+$ ]]; then
      ...
      case "${convert_type}" in
        '1')
          echo -n -e "${Color_Off}The result is: ${BGreen}"; echo "$((2#${binary_value}))"; echo -e -n "${Color_Off}" 
          ;;
        '2') 
          echo -n -e "${Color_Off}The result is: ${BGreen}";printf '%x\n' "$((2#$binary_value))"; echo -e -n "${Color_Off}"
          ;;
        '3') 
          echo -n -e "${Color_Off}The result is: ${BGreen}";echo "${binary_value}" | perl -lape '$_=pack"(B8)*",@F'; echo -e -n "${Color_Off}" 
          ;;
      esac
    else
  ...
  if [[ "${decimal_value}" =~ $re ]]; then
    ...
    case "${convert_type}" in
      1)
        echo -n -e "${Color_Off}The result is: ${BGreen}"; echo "obase=2;${decimal_value}" | bc; echo -e -n "${Color_Off}" 
        ;;
      2) 
        echo -n -e "${Color_Off}The result is: ${BGreen}"; printf '%x\n' "${decimal_value}"; echo -e -n "${Color_Off}" 
        ;;
      3) 
        echo -n -e "${Color_Off}The result is: ${BGreen}"; printf '%s' "$(printf '\\%03o' "${decimal_value}")"; echo -e -n "{Color_Off}" 
        ;;
    esac
  else
  ...
  if [[ $result_hex_text =~ ^[^[:cntrl:]]*$ ]]; then
    case "${convert_type}" in
      1) 
        echo -n -e "${Color_Off}The result is: ${BGreen}"; echo "$result_hex_bin"; echo -e -n "${Color_Off}"
        ;;
      2) 
        echo -n -e "${Color_Off}The result is: ${BGreen}"; echo $((16#${hex_value}));echo -e -n "${Color_Off}"
        ;;
      3) 
        echo -n -e "${Color_Off}The result is: ${BGreen}"; echo "$result_hex_text"; echo -e -n "${Color_Off}"
        ;;
    esac
  else
  ...
  case "${convert_type}" in
    1) 
      echo -n -e "${Color_Off}The result is: ${BGreen}"; echo "${text}" | perl -lpe '$_=join " ", unpack"(B8)*"';  echo -e -n "{Color_Off}"
      ;;
    2)
      echo -n -e "${Color_Off}The result is: ${BGreen}"; printf '%d' "'${text}"; echo ""; echo -e -n "${Color_Off}"
      ;;
    3)
      echo -n -e "${Color_Off}The result is: ${BGreen}"; echo "${text}" | xxd -ps -c 200; echo -e -n "${Color_Off}"
      ;;
  esac
else
